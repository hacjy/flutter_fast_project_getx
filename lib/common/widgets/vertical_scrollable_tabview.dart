...
class VerticalScrollableTabView extends StatefulWidget {
  ...
  Null Function(dynamic index) onChange;
}
...
class _VerticalScrollableTabViewState extends State<VerticalScrollableTabView>
  ...
  @override
  void initState() {
    widget._tabController.removeListener(tabListen);
    widget._tabController.addListener(tabListen);
  ...
  void tabListen() {
    ...
    if (VerticalScrollableTabBarStatus.isOnTap) {
      widget.onChange(VerticalScrollableTabBarStatus.isOnTapIndex);
  ...
  bool onScrollNotification(ScrollNotification notification) {
    ...
    //print('lastTabIndex $lastTabIndex');
    ...
    if (reachLastTabIndex) {
      ...
      // print(
      //     'middleIndex $middleIndex $sumIndex  ${visibleItems.length} ${visibleItems}');
      if (widget._tabController.index != middleIndex) {
        widget.onChange(visibleItems[0]);
    ...
    itemsKeys.forEach((index, key) {
      ...
      switch (isHoriontalScroll) {
        ...
        default:
          ...
          // 如果 item 下方的座標 比 listView 的上方的座標 的位置的小 代表不在畫面中。
          if (itemRect.bottom - 100 < rect.top) return;
      }
