...
Future<BitmapDescriptor> createCustomMarkerBitmap(String title,
  ...
  painter.layout();
  painter.paint(canvas, const Offset(20.0, 20.0));
  int textWidth = painter.width.toInt();
  int textHeight = painter.height.toInt();
  canvas.drawShadow(
      Path()
        ..addRect(Rect.fromPoints(
            Offset(10, 10), Offset(textWidth + 43, textHeight + 20))),
      Color.fromARGB(80, 39, 39, 39),
      3,
      false);
  canvas.drawRRect(
      RRect.fromLTRBAndCorners(10, 20, textWidth + 40, textHeight + 20,
  ...
  ByteData? pngBytes = await (await p.toImage(
          painter.width.toInt() + 50, painter.height.toInt() + 80))
