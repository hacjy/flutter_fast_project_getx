  ...
  static Future<bool> launchQuery(String query) async {
    var googleUri =
        Uri(scheme: 'geo', host: '0,0', queryParameters: {'q': query})
            .toString();
    if (Platform.isIOS) {
      googleUri = 'comgooglemapsurl://maps.google.com/' +
          Uri(queryParameters: {
            'q': query,
            "f": 'd',
    ...
    var iosUri =
        Uri.https('maps.apple.com', '/', {'q': query, "f": 'd'}).toString();
    var g = await launch(googleUri);
    if (!g && Platform.isIOS) {
      return launch(iosUri);
    }
    return g;
    /*    if (!g) {
      return launch(iosUri);
